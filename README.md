...
**Requirements:** Node.js v14.21.1 or later 
...
- `authorised → failed` is not implemented as the brief only specifies `pending → failed`. See future improvements.
...
Valid `status` values: `pending`, `authorised`, `captured`, `refunded`, `failed`.
...
The Express server boilerplate was generated by Claude Code from a screenshot of my Excalidraw planning diagram (see file in project), which covered core entities, API design, and the payment flow, and instructions on architecture and tech stack choices. I then made some changes to the generated code, outlined in the sections below. Claude LLM was also used for guidance and trade-offs throughout.
...
- Tests and demo script scaffolding
- ReadME scaffolding
...
- **`authorised → failed` transition** — sensible in a real system (authorisations can expire or be voided) but not specified in the brief so not implemented.
...
- **`GET /payments` clinic validation** — filtering by `clinicId` currently returns an empty array for an unknown clinic. With a dedicated clinic entity, this would return a 404 if the clinic doesn't exist.
- **`GET /ledgers/:clinicId` for unknown clinic** — currently returns a 200 with zeroes and an empty array, since there is no clinic entity in the data model. With a dedicated clinic table, this would look up the clinic first and return a 404 if it doesn't exist.