...
`idempotencyKey` and `requestHash` are stored directly on the `payments` table rather than in a separate table. Given idempotency is only required for payment creation, a separate table would be unnecessary complexity. See trade-offs below.
